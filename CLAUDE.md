...
1. **My Lists (главный экран)** — список карточек по дням, новые сверху. Наверху — сворачиваемый виджет Progress Overview (бар-чарт за последние дни). При отсутствии данных — empty state с кнопкой «Create First List».
2. **Create New List** — поля: имя списка, список задач с чекбоксами. На вебе — центрированная карточка, на мобиле — полноэкранный вид.
3. **Progress View (просмотр/редактирование дня)** — заголовок с кнопкой «Back» и счётчиком выполненных задач. Экран разделён на «Pending» и «Completed». При клике на checkbox задача перечёркивается и перемещается между секциями. У каждой задачи — кнопка редактирования (карандаш).
...
- Ширина макета: 1280px; Create List — центрированная карточка поверх фона
- Дизайн (светлая + тёмная тема, mobile + web) — `pencil-design.pen`
