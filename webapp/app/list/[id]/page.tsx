...
function TaskRow({ task, onToggle }: TaskRowProps) {
  return (
    <div className="flex items-center gap-3 py-3 border-b border-[var(--divider)]">
      <div
      ...
      <Button variant="ghost" size="icon" data-testid={`task-edit-${task.id}`}>
      ...
      >
        ...
        {pending.length > 0 && (
          <>
            ...
            {pending.map((task) => (
              <TaskRow key={task.id} task={task} listId={id} onToggle={handleToggle} />
            ))}
          </>
        )}
